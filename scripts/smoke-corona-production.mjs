...
try {
  ...
  if(!/Corona Extra Price Map/i.test(heading)) throw new Error(`unexpected heading=${heading}`);
  ...
  console.log(`PRODUCTION SMOKE PASS url=${base} fresh=${current.freshCountryCount} rows=${rows} mapPaths=${paths} detail=${title} mobileOverflow=${overflow}`);
} finally {
  await browser.close();
}
