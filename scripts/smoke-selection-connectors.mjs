...
try{
  ...
  if(await page.locator('style[data-selection-connectors="r1"]').count()!==1)throw new Error('selection connector stylesheet missing');
  ...
  if(await page.locator('.selection-connector-layer path[data-connector-leg="spotlight-ranking"][data-connector-complete="1"]').count()!==1)throw new Error('spotlight → ranking connector missing');
  ...
  if(await page.locator('.selection-connector-layer path[data-connector-leg="spotlight-map"][data-connector-complete="1"]').count()!==1)throw new Error('spotlight → map connector missing');
  if((await page.locator('#country-title').innerText()).trim()!=='Canada')throw new Error('Canada spotlight did not update');
  ...
  if(await page.locator('.mobile-selection-rail.to-ranking').count()!==1)throw new Error('mobile vertical rail did not reach ranking stage');
  if(await page.locator('#ranking tr.connector-selected-row.connector-mobile-reveal-row').filter({hasText:'Australia'}).count()!==1)throw new Error('mobile selected ranking row missing');
  ...
  console.log(`SELECTION CONNECTOR SMOKE PASS desktop=map→spotlight→ranking+ranking→spotlight→map mobile=vertical-rail overflow=${overflow}`);
} finally {
